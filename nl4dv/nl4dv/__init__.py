...
class NL4DV:
    ...
    def render_vis(self, query_raw):
        # type: (str) -> VegaLite
        response = self.analyze_query(query_raw, debug=True)
    ...
    def analyze_query(self, query_raw, dialog=False, debug=False):
        ...
        # DETECT EXPLICIT VISUALIZATION UTTERANCES(Quda not include)
        st = time.time()
        ...
        #task analysis based on model improved by quda
        task_map = self.task_genie_instance.extract_tasks_from_model(encodeable_attributes, self.query_processed, self.dependencies, self.query_tokens)
        ...
        st = time.time()
        # Final list of encodeable attributes in the VIS for barchart and boxplot that need to be highlight
        final_encodeable_attributes_for_highlight = self.attribute_genie_instance.update_encodeable_attributes_based_on_tasks(key = True)
        self.vis_list_for_highlight = self.vis_genie_instance.get_vis_list(attribute_list=final_encodeable_attributes_for_highlight, task_list=self.extracted_tasks, key=True)
        #print(final_encodeable_attributes_for_highlight)
        # Final list of encodeable attributes in the VIS
        final_encodeable_attributes = self.attribute_genie_instance.update_encodeable_attributes_based_on_tasks(key = False)
        print(final_encodeable_attributes)
        self.vis_list = self.vis_genie_instance.get_vis_list(attribute_list=final_encodeable_attributes, task_list=self.extracted_tasks, key=False)
        #print(self.vis_list_for_highlight)
        for item in self.vis_list:
            if item['vlSpec']['mark']['type'] == 'bar' or item['vlSpec']['mark']['type'] == 'boxplot':
                self.vis_list.remove(item)
        for item in self.vis_list_for_highlight:
            if item['vlSpec']['mark']['type'] == 'bar' or item['vlSpec']['mark']['type'] == 'boxplot':
                self.vis_list.append(item)
        print(self.vis_list)
...
#     para = NL4DV(data_url="movies-w-year.csv")
#     para.render_vis('Create a histogram showing distribution of IMDB ratings.')
